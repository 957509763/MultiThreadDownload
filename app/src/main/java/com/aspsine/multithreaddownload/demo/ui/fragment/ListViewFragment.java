...
public class ListViewFragment extends Fragment implements OnItemClickListener<AppInfo> {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mAppInfos = DataSource.getInstance().getData();//获得APP信息
    ...
    public void onItemClick(View v, final int position, final AppInfo appInfo) {
        if (appInfo.getStatus() == AppInfo.STATUS_DOWNLOADING || appInfo.getStatus() == AppInfo.STATUS_CONNECTING) {  //如果应用“正在下载”、“连接中”设置暂停
            pause(appInfo.getUrl());
        } else if (appInfo.getStatus() == AppInfo.STATUS_COMPLETE) {     //如果应用“完成”、设置安装
            install(appInfo);
